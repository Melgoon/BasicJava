...
class Outer{
	...
	//인스턴스 변수 static가 안붙은 변수를 인스턴스 변수
	int instanceVar = 1;
	int var = 10;
	
	//클래스 변수 static가 붙은 변수를 클래스 변수
...
//		int inner1 = instanceVar; //메모리에 아직 올라오지 않았기 때문에 사용 불가
		int inner2 = classVar;
		static int inner3 = classVar;
	}
	void method(){
		//지역 변수 메소드 안에 있는 변수를 지역변수
		int localVar1 = 3;
		final int localVar2 = 4;
		
		//지역 클래스 메소드 안에 있는 클래스를 지역 클래스
		class LocalInner{
			//지역 클래스에서는 인스턴스 멤버와 클래스 멤버 모두 사용 가능하고
			//funal이 붙은 지역 변수도 사용 가능
			int inner1 = instanceVar;
			int inner2 = classVar;
//			int inner3 = localVar1; final이 붙지 않아 사용 불가
